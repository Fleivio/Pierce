...
termToNamed term =
  case term of
    ...
    Fst x -> termToNamed x `N.App` termToNamed TT
    Snd x -> termToNamed x `N.App` termToNamed FF
    Pair t1 t2 -> N.Abs "@selector" (N.Var "@selector" `N.App` termToNamed t1 `N.App` termToNamed t2)
    Numb k -> toChurch k
    ...
    IsZero m -> termToNamed m `N.App` N.Abs "_" (termToNamed FF) `N.App` termToNamed TT
